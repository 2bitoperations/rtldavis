...
class ByteToCmplxLUT:
    ...
    def execute(self, in_bytes: np.ndarray, out_cmplx: np.ndarray) -> None:
        ...
        if in_bytes.size != out_cmplx.size * 2:
            logger.error(
                f"Incompatible array sizes: in_bytes.size={in_bytes.size}, out_cmplx.size={out_cmplx.size}"
            )
...
class PacketConfig:
    def __init__(
        ...
        self.buffer_length = (
            self.packet_length // self.block_size + 2
        ) * self.block_size
    ...
    def demodulate(self, input_data: np.ndarray) -> List[Packet]:
        ...
        if np.iscomplexobj(input_data):
            if input_data.size != dest.size:
                logger.error(
                    f"Incompatible array sizes: input_data.size={input_data.size}, dest.size={dest.size}"
                )
                raise ValueError("Incompatible array sizes")
            dest[:] = input_data
        else:
        ...
        quantize(
            self.discriminated[self.cfg.block_size :],
            self.quantized[self.cfg.buffer_length - self.cfg.block_size :],
        )
        ...
        for q_idx in indices:
            ...
            if pkt_bytes not in seen:
                ...
                if noise_end > noise_start:
                    ...
                    noise_power = (
                        1e-9  # Avoid division by zero if no noise region is available
                    )

                # Estimate signal power from the preamble region
                preamble_iq = self.filtered[
                    signal_start : signal_start + self.cfg.preamble_length
                ]
                ...
                packets.append(
                    Packet(
                        index=q_idx,
                        data=np.frombuffer(pkt_bytes, dtype=np.uint8),
                        rssi=rssi,
                        snr=snr,
                    )
                )
